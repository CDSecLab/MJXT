...
public class test_JXTpp {
    ...
    public static void main(String[] args) {
        ...
        String condition = "";//the condition aims to choose different tables from dataset
        ...
        for (int v = 0; v < 20; v++) {
            for (int i = 0; i < table_num; i++) {
                keyword[i] = "keyword0" + "table" + (i + 1) + "_keyword_" + v + "_0";
            ...
            for (int x = 0; x < 1000; x++) {//run 1000 times
                ...
                for (int i = 0; i < res.size(); i++) {
                    ...
                    for (int j = 0; j < res_i.length; j++) {
                        if (AESUtil.decrypt(k_dec[i % table_num], res_i[j]) != null) res_size++;
                    }
                }
